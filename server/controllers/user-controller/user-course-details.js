...
const getUserCourseDetails = async (req, res) => {
  try {
    ...
    const extractProgressOnEachCourse = async (
      ...
      const eachCourseLectureStatus = async (eachCourse) => {
        let lectureProgress = {};
        ...
        if (eachCourseId === targetCourseId) {
          ...
          lectureProgress["totalLectures"] = eachCourse.lecturesProgress.length;
      ...
      let eachCourse = UserCoursesProgress.filter(
        (eachCourseItem) => eachCourseItem.courseId === targetCourseId
      );
      // console.log("FoundCourse: ", eachCourse[0])
      let lectureProgress = await eachCourseLectureStatus(eachCourse[0]);
